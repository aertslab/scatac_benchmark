...
intermediate_dir = 'fmx_out/intermediate'
...
for n,(fmxVCFname,fmxVCF) in enumerate(f_fmx_vcfs.items()):
    for sample in fmx_samples:
        ...
        cmd = f"bcftools view -s {sample} -i 'MIN(FMT/DP)>{min_DP} & MIN(FMT/GQ)>{min_GQ}' {fmxVCF} -O v | bcftools reheader -s fmx_out/sample_rename | bcftools view -O z -o {intermediate_dir}/{f_vcf} && tabix -p vcf -f {intermediate_dir}/{f_vcf}"
        print(cmd)
